...
var (
	...
	groupID     string
	controller  *handler.LineController
)

func TestMain(m *testing.M) {
	mockService = &MockService{}
	groupID = "groupID"
	controller = handler.NewLineController(groupID, mockService)
	run := m.Run()
	os.Exit(run)
}
...
func TestLineController_Check(t *testing.T) {
	...
	util.SetStatus("existTrueUserID", "true")
	util.SetStatus("existFalseUserID", "false")
